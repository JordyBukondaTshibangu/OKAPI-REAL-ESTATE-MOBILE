...
function demandLevel(score: number): { label: string; ratio: number; color: string } {
  if (score >= 75) return { label: "Demande forte", ratio: 1, color: "#16A34A" };
  if (score >= 25) return { label: "Demande modérée", ratio: Math.min(score / 100, 0.66), color: "#F59E0B" };
  return { label: "Demande faible", ratio: Math.max(score / 100, 0.04), color: "#DC2626" };
}
...
export default function PerformanceCard({ performance, isDark }: Props) {
  ...
  const { label, ratio, color } = demandLevel(demandScore(performance));

  const stats = [
    { icon: Eye,    value: performance.viewed, label: "Vues",     tint: isDark ? Colors.dark.primary : Colors.primary, bg: isDark ? Colors.dark.accent : Colors.accent },
    { icon: Share2, value: performance.shared, label: "Partages", tint: "#0E9BB5", bg: isDark ? "#11343F" : "#E0F5FA" },
    { icon: Heart,  value: performance.saved,  label: "Favoris",  tint: "#E8336E", bg: isDark ? "#3F1828" : "#FDE8EF" },
  ];
  ...
  return (
    <View style={{ backgroundColor: cardBg, paddingHorizontal: 20, paddingVertical: 16, marginBottom: 8 }}>
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 16, marginBottom: 8 }}>
        <Text style={{ color: textMuted, fontSize: 14 }}>Niveau de demande</Text>
        <Text style={{ color: textMain, fontFamily: "DMSans_700Bold", fontSize: 14 }}>{label}</Text>
      </View>
      <View style={{ height: 6, borderRadius: 3, backgroundColor: trackBg, overflow: "hidden" }}>
        <View style={{ height: 6, borderRadius: 3, width: `${Math.round(ratio * 100)}%`, backgroundColor: color }} />
      </View>
    </View>
  );
}
